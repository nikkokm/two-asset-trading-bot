...
def get_prices(start, end, frequency='daily'):
    """
    - Pass a single ticker (string) or a list of tickers.
    - Pass tickers in uppercase
    - The function returns a date-stamped dataframe with prices
    - start & end are are strings in 'yyyy-mm-dd' format passed directly.
    - AlphaVantage has questionable data integrity. Better to always use daily data and resample if necessary
    - frequency takes 'daily' by default (recommended) but can also take 'weekly' and 'monthly.
    - All prices are adjusted closing prices.
    """


    tickers = TICKERS  # fetch tickers from config.py
    frequency = frequency.upper()  # This was pointless, in hindsight.

    df_final = pd.DataFrame()  # declared for merging purposes (inside loops)

    if type(tickers) == str:  # In case a string passed,
        tickers = list([tickers])  # cast it to a list
